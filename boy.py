...
async def send_join_message(event):
    # Check if it's a callback query - use edit else respond
...
async def main():
    ...
    try:
        ...
        if "database is locked" in error_msg or "unable to open database file" in error_msg:
            ...
            # Uncomment if you want to auto-delete session file (use with caution)
            # session_file = bot.session.filename if hasattr(bot.session, 'filename') else None
            # if session_file and os.path.exists(session_file):
            #     os.remove(session_file)
            #     logging.info("Session file deleted, retrying...")
            #     await bot.start(bot_token=BOT_TOKEN)
            # else:
            #     return
        else:
