...
def build_clone_env(bot_token: str, franchise_id: str, owner_id: int, display_name: str) -> str:
    """Pre-filled .env for a franchise partner — shared infra copied from this
    master instance, everything franchise-specific left for them to set."""
    return (
        "# ===== Auto-generated by Clone Provisioning =====\n"
        f"BOT_TOKEN={bot_token}\n"
        f"API_ID={API_ID}\n"
        f"API_HASH={API_HASH}\n"
        f"MONGO_URL={MONGO_URL}\n\n"
        f"FRANCHISE_ID={franchise_id}\n"
        f"FRANCHISE_OWNER_ID={owner_id}\n"
        f"BOT_DISPLAY_NAME={display_name}\n"
        f"ADMIN_IDS={owner_id}\n\n"
        f"SMM_API_URL={SMM_API_URL}\n"
        f"SMM_API_KEY={SMM_API_KEY}\n\n"
        "# --- Franchise sets these themselves (their own payment collection) ---\n"
        "UPI_ID=youruniqueid@upi\n"
        "PAYEE_NAME=Your Name\n\n"
        "# --- Optional ---\n"
        "LOGS_CHANNEL_ID=\n"
        "DEFAULT_MARKUP_PERCENT=20\n"
        "TG_MEMBER_MARKUP_PERCENT=50\n"
        "REFERRAL_BONUS_PERCENT=10\n"
        "REFERRAL_BONUS_MAX=5\n"
        "MIN_DEPOSIT=10\n"
    )
