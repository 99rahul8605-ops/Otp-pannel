...
async def _server1_pick_active_replacement(
    *,
    ...
    for query in queries:
        cursor = accounts_col.find(query).sort([("price", 1), ("_id", 1)])
...
async def callback_handler(event):
    try:
        ...
        if data == "buy":
            manual_count = await accounts_col.count_documents({"status": "available"})
            buttons = []
        ...
        if data == "buy_manual":
            ...
            buttons = [
                [Button.inline(c, f"country_{c}", style="primary")]
                for c in countries
            ]
            buttons.append([Button.inline("🔙 Servers", b"buy", style="primary")])
        ...
        if data.startswith("vnh_country_"):
            ...
            await event.edit(
                ...
                "💰 The amount will only be deducted from your account when you successfully "
                "receive the login codes.\n\n"
                "⚠️ Please note that cancellation is not available on this server because "
                "OTP delivery is guaranteed.\n\n"
                "⚠️ **Terms & Conditions**\n"
        ...
        if data == "vnh_confirm_purchase":
            ...
            old_withdrawable = float(user.get("withdrawable_balance", 0))
            new_withdrawable = max(0, old_withdrawable - retail_price)

            deducted = await users_col.update_one(
                {
                    "user_id": user_id,
                    "balance": {"$gte": retail_price},
                },
                {
                    "$inc": {"balance": -retail_price},
                    "$set": {"withdrawable_balance": new_withdrawable},
                },
            )
            ...
            data_obj = order_response.get("data") or {}
            phone = str(
                data_obj.get("number")
            ...
            if not phone:
                # Do not retry a purchase request automatically.
                await users_col.update_one(
                    {"user_id": user_id},
                    {"$inc": {"balance": retail_price}},
                )
        ...
        if data.startswith("vnh_otp_"):
            ...
            otp = str(otp_data.get("code") or "").strip()
